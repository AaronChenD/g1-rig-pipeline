...
| `--pose` | `zero` | 绑定姿势：`zero` = URDF 官方零位（默认，手臂下垂）/ `tpose` = T-Pose 双肩外展（动捕重定向可选起手） |
...
- **绑定姿势 = URDF 官方零位**（手臂自然下垂，机器人真值回放零补偿），**双脚贴地站在网格上**：脚底 Z=0、pelvis 在 Z≈0.79 m、总高 1.32 m（`--pose tpose` 可切换动捕重定向常用的 T-Pose 起手；`--no-ground` 可退回 URDF 原生 pelvis 原点）
...
| `<名称>_houdini.usda` | **Houdini 21** | 米 | Y-up | 面向 +Z | 原生米制；**数据直接烘成 Y-up、根变换恒等**——`USD Character Import` 的模型/骨骼/动画三个输出方向完全一致（固定 `.usda` 文本格式） |
...
- **三份均为 URDF 官方零位绑定（手臂下垂）+ 双脚贴地**（脚底在 0、pelvis 在 79.2cm）——机器人真值回放零补偿；偏好 T-Pose 起手做动捕重定向的话加 `--pose tpose`。
- 扩展名说明：`--usd` 写成 `.usdc`/`.usdz` 时 `_ue` 兄弟文件跟随该格式；`_houdini` 固定 `.usda`（导出后要修补 upAxis 元数据，Blender 内置 Python 没有 pxr，只能文本替换）。
...
2. **Blender 导出器把 `restTransforms` 写成了父级相对的局部变换**（USD 规范要求它与 bindTransforms 一样是骨架空间绝对值）。`USD Character Import` 的模型/骨骼输出按规范消费 rest、动画输出用 bind——局部值被当绝对值，骨架全部缩回原点附近（表现为 pelvis 在原点、差 ±90° 朝向、位置对不齐）。脚本导出后会把 restTransforms **补丁成 bindTransforms**（绑定姿势导出的文件里两者本应相等），三个输出完全一致。
3. **Skeleton 没有 `skel:animationSource` 动画绑定**（节点会警告 "does not have an animation binding"）。没有绑定时动画输出回退用 bind（所以它一直是对的），但模型/骨骼输出走 rest 路径，任何数据不一致都会被放大。脚本现在会在 SkelRoot 里自动生成一个 `bind_pose` **SkelAnimation** prim（62 关节的局部平移/四元数，t=0 一帧，内容=绑定姿势），并在 Skeleton 上挂 `skel:animationSource` 指向它——三个输出从此走同一份数据。这里还有两个 USD 规范的暗坑（都踩过）：动画属性必须写成 **timeSamples**（无采样的默认值会被 pxr/Houdini 的 AnimQuery 忽略）；`scales` 的 schema 类型是 **half3[]** 而非 float3[]（类型不符整个动画静默无效）。
...
>
> 旧文件（2025-09 之前生成的）如果只有动画输出方向对：重新跑一次脚本生成新 `_houdini.usda` 即可；或临时给模型/骨骼两路加 Transform SOP 旋转 90°。
...
**Q17：骨骼位置好像不太对？（头部的骨头戳在骨盆/腰部，转头时整个上身跟着动）**
这是宇树官方 G1 URDF 的一个"坑"，不是你操作错。诊断结论：`head_joint` 和 `logo_joint`（都是 fixed 关节）的 origin 写的是 `xyz="0.0039635 0 -0.044"`，沿腰部链条累算后**关节坐标系落回骨盆附近 (0,0,0)**；但这两条 link 的 STL 却是全局装配坐标风格（顶点直接是整机位置）。结果就是：网格摆得没问题，骨骼却戳在骨盆——脖子里的骨骼轴心不在脖子里，转脖子自然整身乱动。（MJCF 版 `g1_29dof_rev_1_0.xml` 数字完全一样，属于官方模型自带，非解析 bug。）

本仓库脚本已内置修正，构建时会打印 `[fix] head_link, logo_link`，规则：

- **fixed 关节**且关节原点落在自身 mesh 包围盒外（留 0.03 m 余量）→ 判定为"STL 全局坐标"型错位，骨骼 head 改用 **mesh 包围盒底面中心**（正好落在脖根/零件根部，姿态轴不变，只挪位置）；
- **revolute / continuous / prismatic 一律保持关节原点不动**（保护动画旋转轴，膝/髋等零件的自然延伸不算错位）。
...
- **绑定姿势是 URDF 官方零位（手臂下垂）、双脚贴地**（pelvis 在 79.2cm）——reference pose 与机器人官方状态一致；IK Retargeter 偏好 T-Pose 参考的话用 `--pose tpose` 重导一份。
...
**Q19：机器人的初始摆位/姿势是什么样的？为什么 pelvis 不在原点？**
两件事，脚本默认都处理好（与官方一致 + DCC 角色惯例）：
...
2. **绑定姿势 = URDF 官方零位**（手臂自然下垂，机器人真值回放/关节角补偿为零）。动捕重定向流程偏好 T-Pose 起手的话，加 `--pose tpose` 重新导出——肩 roll 偏移（左 +90°/右 -90°）会记录在 meta JSON 的 `pose.joint_offsets_deg`，机器人真值回放时按 `urdf 关节角 = 骨骼局部旋转 + 偏移` 补偿。
